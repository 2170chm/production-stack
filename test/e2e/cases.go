...
var CaseDeployments = map[string][]deploy.ModelDeploymentValues{
	...
	CasePrefixCachePerf: {
		{
			// GPU-mocked prefix-cache perf case. Runs on the
			// gpu-node-mocker path (fake nodes + llm-d-inference-sim shadow
			// pods, no real GPU), so it needs no A100 capacity. The sim runs
			// with enable-kvcache + block-size 16 and the built-in (dummy)
			// tokenizer, which still yields deterministic per-block hashes, so
	...
	CaseControlPlaneError: {
		{
			...
			// is enough because the specs scale only control-plane Deployments.
